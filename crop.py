...
def process(img, new_width, new_height):
    ...
    if horizontal:
        image.thumbnail(size=(image.size[0], new_height), resample=Image.Resampling.LANCZOS)
        image = crop(image, new_width, new_height)
        save(image, img)
    elif vertical:
        image.thumbnail(size=(new_width, image.size[1]), resample=Image.Resampling.LANCZOS)
        image = crop(image, new_width, new_height)
        save(image, img)
    else:
